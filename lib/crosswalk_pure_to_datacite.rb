module CrosswalkPureToDatacite
  ...
  def crosswalk_pure_native_to_datacite_dataset_metadata(doi, pure_dataset_metadata)
    ...
    builder = Nokogiri::XML::Builder.new(:encoding => 'UTF-8') do |xml|
      xml.resource( 'xmlns' => 'http://datacite.org/schema/kernel-3',
        ...
        xml.titles {
          xml.title doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":title/core:localizedString", ns).text
        }
        description_path = pure_dataset_xpath_prefix + pure_dataset_response_type + ":descriptions//extensions-core:value/core:localizedString"
        ...
        xml.publisher doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":publisher//publisher-template:name", ns).text

        t = Time.parse(doc.xpath(pure_dataset_xpath_prefix + "core:created", ns).text)
        xml.publicationYear t.strftime("%Y")

        keyword_group_path = pure_dataset_xpath_prefix + "core:keywordGroups/core:keywordGroup/core:keyword/core:userDefinedKeyword/core:freeKeyword"
        ...
        xml.alternateIdentifiers {
          xml.alternateIdentifier doc.xpath(pure_dataset_xpath_prefix + "@uuid", ns).text, :alternateIdentifierType => 'Pure UUID'
        }
        xml.dates {
          # available
          year = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":dateMadeAvailable/core:year", ns).text
          month = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":dateMadeAvailable/core:month", ns).text
          day = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":dateMadeAvailable/core:day", ns).text
          ymd = ''
          if !year.empty?
            ymd << year
          end
        ...
        locale = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":title/core:localizedString/@locale", ns).text
        locale = locale.gsub('_', '-').downcase
        xml.language locale

        geoLocationPlace = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":geographicalCoverage/core:localizedString", ns).text
        ...
        # sizes
        sizes = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":documents//core:size", ns)
        if !sizes.empty?
          xml.sizes {
            sizes.each do |size|
              xml.size size.text
        ...
        # formats
        formats = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":documents//core:mimeType", ns)
        if !formats.empty?
          xml.formats {
        ...
        licenses = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":documents//" + pure_dataset_response_type + ":documentLicense", ns)
        if !licenses.empty?
          xml.rightsList {
        ...
        if includeRelatedContent === true
          ...
          relatedPublicationUUIDs = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":relatedPublications/core:relatedContent/@uuid", ns)
  ...
  def date_range_collected(doc)
    ...
    startYear = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":temporalCoverageStartDate/core:year", ns).text
    startMonth = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":temporalCoverageStartDate/core:month", ns).text
    startDay = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":temporalCoverageStartDate/core:day", ns).text
    startDate = ''
    if !startYear.empty?
      startDate << startYear
    end
    ...
    endYear = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":temporalCoverageEndDate/core:year", ns).text
    endMonth = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":temporalCoverageEndDate/core:month", ns).text
    endDay = doc.xpath(pure_dataset_xpath_prefix + pure_dataset_response_type + ":temporalCoverageEndDate/core:day", ns).text
    endDate = ''
    if !endYear.empty?
      endDate << endYear
    end
  ...
  def get_pure_dataset_response_type(doc)
    ...
    return doc.xpath(pure_dataset_xpath_prefix + "@xsi:type", ns).text.split(":")[0]
  end

  def pure_dataset_xpath_prefix
    "dataset:GetDataSetsResponse/core:result/core:content/"
  end
